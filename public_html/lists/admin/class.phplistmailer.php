...
spl_autoload_register(
    function ($classname)
    {
        ...
        if (substr($classname, 0, $prefixLength) == $prefix) {
            $filename = 'PHPMailer/src/' . substr($classname, $prefixLength) . '.php';
            require $filename;
        }
    }
);

class PHPlistMailer extends PHPMailer\PHPMailer\PHPMailer
{
...
//      cl_output('get file system image'.$filename.' '.$localfile);
    ...
    public function amazonSesSend($header, $body)
    {
        ...
        $rawmessage = base64_encode($header.static::$LE.static::$LE.$body);
        $requestdata = array(
            'Action'                => 'SendRawEmail',
