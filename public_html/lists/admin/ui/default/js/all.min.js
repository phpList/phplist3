...
conf=$.extend({},$.tools.scrollable.conf,conf);this.each(function(){el=new Scrollable($(this),conf);$(this).data("scrollable",el);});return conf.api?el:this;};})(jQuery);var busyImage='<img src="images/busy.gif" with="34" height="34" border="0" alt="Please wait" />';var menuArrowImage='ui/default/images/menuarrow.png';var menuArrowActiveImagesrc='ui/default/images/menuarrow_active.png';function urlParameter(name,link){name=name.replace(/[\[]/,"\\\[").replace(/[\]]/,"\\\]");var regexS="[\\?&]"+name+"=([^&#]*)";var regex=new RegExp(regexS);var results=regex.exec(link);if(results==null)
return"";else
return results[1];}
...
return true;});var docurl=document.location.search;document.cookie="browsetrail="+escape(docurl);setInterval("autoSave();",120000);$('li.selectallcategory').on('click',function(){if($(this).find('input[type=checkbox]').attr('id').match('all-lists')){var ul=$(this).parent();var lists=ul.parent().find('li');checkAllBoxes(lists.find('input[id^=all-lists]').prop('checked'),lists);}});setInterval(getServerTime,30000);});function allDone(text){if(!text){text='All done';}
