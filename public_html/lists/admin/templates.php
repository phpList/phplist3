...
if (!Sql_Affected_Rows()) {
    echo '<p class="information">'.$GLOBALS['I18N']->get('No template have been defined').'</p>';
}
...
while ($row = Sql_fetch_Array($req)) {
    ...
    $ls->addRow($element, $img_template,
        '<span class="button">'.PageLinkDialogOnly('viewtemplate&amp;id='.$row['id'],
            $GLOBALS['img_view']).'</span>'.sprintf('<span class="delete"><a class="button" href="javascript:deleteRec(\'%s\');" title="'.$GLOBALS['I18N']->get('delete').'">%s</a>',
            PageUrl2('templates', '', 'delete='.$row['id']), $GLOBALS['I18N']->get('delete')));
...
//  $ls->addColumn($element,$GLOBALS['I18N']->get('View'),);
    $ls->addColumn($element, $GLOBALS['I18N']->get('Campaign Default'),
    ...
    $ls->addColumn($element, $GLOBALS['I18N']->get('System'),
...
if (empty($exists[0])) {
    echo '<p class="button">'.PageLink2('defaultsystemtemplate',
            $GLOBALS['I18N']->get('Add default system template')).'</p>';
}
