...
while ($row = Sql_Fetch_array($res)) {
  ...
  <tr><td colspan="2"><?php echo $GLOBALS['I18N']->get('Attribute:').$row['id'] ?></td><td colspan="2"><?php echo $GLOBALS['I18N']->get('Delete');
    ?> <input type="checkbox" name="delete[<?php echo $row['id'] ?>]" value="1" /></td></tr>
  <tr><td colspan="2"><?php echo $GLOBALS['I18N']->get('Name:');
    ?> </td><td colspan="2"><input type="text" name="name[<?php echo $row['id']?>]" value="<?php echo htmlspecialchars(stripslashes($row['name'])) ?>" size="40" /></td></tr>
  <tr><td colspan="2"><?php echo $GLOBALS['I18N']->get('Type');
    ?> </td><td colspan="2"><input type="hidden" name="type[<?php echo $row['id']?>]" value="<?php echo $row['type']?>" /><?php echo $row['type']?></td></tr>
  <tr><td colspan="2"><?php echo $GLOBALS['I18N']->get('Default Value:');
    ?> </td><td colspan="2"><input type="text" name="default[<?php echo $row['id']?>]" value="<?php echo htmlspecialchars(stripslashes($row['default_value'])) ?>" size="40" /></td></tr>
  <tr><td><?php echo $GLOBALS['I18N']->get('Order of Listing:');
    ?> </td><td><input type="text" name="listorder[<?php echo $row['id']?>]" value="<?php echo $row['listorder'] ?>" size="5" /></td>
  <td><?php echo $GLOBALS['I18N']->get('Is this attribute required?:');
    ?> </td><td><input type="checkbox" name="required[<?php echo $row['id']?>]" value="1" <?php echo $row['required'] ? 'checked="checked"' : '' ?> /></td></tr>
  </table><hr/>
<?php 
} ?>
