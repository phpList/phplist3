...
print '<ul style="list-style:none" class="navigation_list">';
...
if (!$numrules[0]) {
    print '<p class="information text-info"><big>' . $GLOBALS['I18N']->get('You currently have no rules defined.      You can click "Generate Bounce Rules" in order to auto-generate rules from your existing bounces.      This will results in a lot of rules which you will need to review and activate.      It will however, not catch every single bounce, so it will be necessary to add new rules over      time when new bounces come in.') . '</big></p>';
} else {
    print '<p class="information text-warning"><big>' . $GLOBALS['I18N']->get('You have already defined bounce rules in your system.      Be careful with generating new ones, because these may interfere with the ones that exist.') . '</big></p>';
}
print '<br /><p class="button">' . PageLink2('generatebouncerules', $GLOBALS['I18N']->get('Generate Bounce Rules')) . '</p>';
